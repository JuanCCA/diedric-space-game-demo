class Game {
  constructor() {
    ...
    const {
      ...
      width: widthTop,
      height: heightTop,
    } = this.canvasTop.getBoundingClientRect();
    const {
      top: topBottom,
      left: leftBottom,
      width: widthBottom,
      height: heightBottom,
    } = this.canvasSide.getBoundingClientRect();
  ...
  drawScene(view, ctx) {
    ...
    this.planets.forEach((p) =>
      drawables.push({ obj: p, depth: getDepth(p.position) }),
    );
  ...
  updateUI() {
    // Ship stats
    document.getElementById("speedX").textContent =
      this.ship.velocity.x.toFixed(2);
    document.getElementById("speedY").textContent =
      this.ship.velocity.y.toFixed(2);
    document.getElementById("speedZ").textContent =
      this.ship.velocity.z.toFixed(2);
    document.getElementById("posX").textContent =
      this.ship.position.x.toFixed(2);
    document.getElementById("posY").textContent =
      this.ship.position.y.toFixed(2);
    document.getElementById("posZ").textContent =
      this.ship.position.z.toFixed(2);

    // Camera stats
    document.getElementById("cameraScale").textContent =
      this.camera.scale.toFixed(2);
    document.getElementById("cameraFollow").textContent = this.camera.followShip
      ? "Sí"
      : "No";
    ...
    this.planets.forEach((planet) => {
      const screenX =
        this.canvasTop.width / 2 +
        (planet.position.x - this.camera.x) * this.camera.scale;
      const screenY =
        this.canvasTop.height / 2 +
        (planet.position.y - this.camera.y) * this.camera.scale;
      if (
        screenX > -planet.radius &&
        screenX < this.canvasTop.width + planet.radius &&
        screenY > -planet.radius &&
        screenY < this.canvasTop.height + planet.radius
      ) {
        visiblePlanets++;
      }
    });
